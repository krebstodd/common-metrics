...
public class InstrumentedJettyServer extends Server {
     ...
     */
    public InstrumentedJettyServer(final MetricService metricService, final ThreadPool threadPool, final Consumer<HttpChannel<?>> channelHandler) {

        super(threadPool);

        this.txFactory = metricService.transactionFactory().inGroup(EventGroup.SERVER_HTTP).withName("http-response").inDirection(Direction.INBOUND).build();
